...
public class myPosition implements Parcelable{
    ...
    public myPosition(JSONArray array) {
    ...
    public myPosition(double[] array) {
        if (array.length == 2) {
    ...
    public myPosition(double latitude, double longitude, double altitude) {
        this.mStorage[LAT_IDX] = latitude;
        this.mStorage[LON_IDX] = longitude;
        this.mStorage[ALT_IDX] = altitude;
    }

    public myPosition(Location location) {
    ...
    private myPosition(Parcel parcel) {
    ...
    public static final Parcelable.Creator<myPosition> CREATOR = new Parcelable.Creator<myPosition>() {
        @Override
        public myPosition createFromParcel(Parcel in) {
            return new myPosition(in);
        }

        @Override
        public myPosition[] newArray(int size) {
            return new myPosition[size];
        }
    };
...
//        coordinates.put(ALT_IDX, this.getAltitude());
    ...
    public boolean equals(Object object) {
        ...
        if (!(object instanceof myPosition)) {
            return false;
        }

        myPosition aPosition = (myPosition) object;
